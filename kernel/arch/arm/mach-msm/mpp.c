...
int config_mpp21_blink_led(unsigned command, unsigned value)
{
        int err;
        err = msm_proc_comm(PCOM_PM_MPP_CONFIG_I_SINK_NOTIFY, &command, &value);
        if(err)
                pr_err("%s: msm_proc_comm(PCOM_PM_MPP21_BLINK_CONFIG) failed!\n",__func__);

        return err;
}
EXPORT_SYMBOL(config_mpp21_blink_led);
